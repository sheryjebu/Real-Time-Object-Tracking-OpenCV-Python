# *** -------------------------------------------------------------------------------------------
# *** Name        : COMMON INTELLIGENCE PLATFORM FOR ROBOTICS
# *** Description : The code implements the Real time Object detection, recognition and tracking
# *** --------------------------------------------------------------------------- -----------------
...
# ------------------------------
# Implementing Opencv DNN Model
# ------------------------------
CIP_net = cv2.dnn.readNet("dnn_model/yolov4-tiny.weights", "dnn_model/yolov4-tiny.cfg")
CIP_model = cv2.dnn_DetectionModel(CIP_net)
CIP_model.setInputParams(size=(320, 320), scale=1 / 255)
...
while 1:
    ...
    if not (targets1 and targets2):
        ...
        if abs(s1 - s2) > maxsd:
            ...
            if len(x1k) >= klen:
                ...
                # triangulate
                X, Y, Z, D = angler.location(camera_separation, (xlangle, ylangle), (xrangle, yrangle), center=True,
                                             degrees=True)
    ...
    if x1k:
        ...
        # Object Detection
        #(class_ids, scores, bboxes) = CIP_model.detect(frame1)
        #(class_ids, scores, bboxes) = CIP_model.detect(frame2)
        ...
        #    # print(x,y,w,h)
        #    cv2.putText(frame1, class_name, (x, y - 10), cv2.FONT_HERSHEY_PLAIN, 2, (200, 0, 50), 2)
        #    cv2.rectangle(frame1, (x, y), (x + w, y + h), (200, 0, 50), 3)

        #    cv2.putText(frame2, class_name, (x, y - 10), cv2.FONT_HERSHEY_PLAIN, 2, (200, 0, 50), 2)
        #    cv2.rectangle(frame2, (x, y), (x + w, y + h), (200, 0, 50), 3)
    # display frame
